...
#include<cstring>
using namespace std;
template<typename T> struct write_type{T data;int index;};
template<class T>class myFileArray{fstream f1;
   public:
   ...
   const T& operator[](const write_type<T> &idx){ // a[complexCls(2,3)]
     f1.seekg(0,ios::end);
     long long int l1=f1.tellg();
     l1 /= sizeof(T);
     if(idx.index>l1){cout<<"Index is out of range\n";exit(0);}
     f1.seekp(idx.index*sizeof(T),ios::beg);
     f1.write((char *) &idx.data , sizeof(T));
     return idx.data;
   }
   void writeNext(T j){
     f1.write((char *)&j, sizeof(T));
...
int main(){myFileArray<student> myi("temp6.in");
  ...
  student st2={"Kamran","68588",12.1};
  write_type<student> wt1{st2,0};
